...
import { agreementTypes, agreementDescriptions, agreementLabels, agreementSchema, type AgreementInput, type AgreementType, type FieldName, BRAND, PROVIDER, filename, formatDate } from '@/lib/agreement';
...
const emptyForm: AgreementInput = {
  ...
  ...PROVIDER, effectiveDate: '', agreements: ['nda', 'msa'],
};
...
export default function DocumentStudio() {
  ...
  const previewParagraphs = activeTab === 'sow' ? sowParagraphs(form) : preview[activeTab];
        ...
        <div className="studio-grid">
          <form ref={formRef} onSubmit={event => { event.preventDefault(); void generate(); }} noValidate className="agreement-form">
            ...
            <section className="form-card"><div className="section-heading"><div className="step-number">03</div><div><h2>Your company details</h2><p>Representing vedryxTech.</p></div><span className="brand-tag">vedryxTech</span></div>
              <div className="provider-details"><strong>{PROVIDER.providerSignatory}</strong><p>{PROVIDER.providerDesignation}, {BRAND}</p><p>{PROVIDER.providerAddress}</p><span>Your signature is automatically added to exported PDFs. The client signs separately.</span></div>
            </section>
            ...
            <div className="generate-panel"><div className="generate-summary"><div className="small-file-stack"><Files size={22} /></div><div><strong>{form.agreements.length} agreement{form.agreements.length === 1 ? '' : 's'} selected</strong><span>Signed PDF {form.agreements.length > 1 ? 'files · downloaded as ZIP' : 'document · .pdf'}</span></div></div>
          ...
          <aside className="preview-column" aria-label="Agreement preview">
            <div className="preview-sticky"><div className="preview-title"><div><span className="live-dot" /><h2>Live preview</h2></div><span>Updates as you type</span></div>
              <div className="preview-frame"><div className="preview-toolbar"><div className="preview-tabs" role="tablist" aria-label="Preview agreement">{agreementTypes.map(type => <button key={type} id={`${type}-tab`} role="tab" aria-selected={activeTab === type} aria-controls="agreement-preview" tabIndex={activeTab === type ? 0 : -1} onKeyDown={event => {
              ...
              }} onClick={() => setActiveTab(type)} className={activeTab === type ? 'current' : ''}>{type.toUpperCase()}</button>)}</div><span><FileText size={13} /> PDF DOCUMENT</span></div>
                <div className="paper-scroll" id="agreement-preview" role="tabpanel" aria-labelledby={`${activeTab}-tab`} tabIndex={0}><article className="paper" key={activeTab}><div className="paper-brand"><Image src="/brand/logo.svg" alt="vedryxTech" width={145} height={29} style={{ height: 'auto' }} /><span>AGREEMENT</span></div><div className="paper-rule" /><div className="paper-kicker">{agreementDescriptions[activeTab].kicker}</div>
                  ...
                  <div className="paper-body">{previewParagraphs.slice(1).map((paragraph, index) => <p key={index} className={`${paragraph.heading ? 'clause-heading' : ''} ${'signature' in paragraph && paragraph.signature ? `signature-${paragraph.signature}` : ''}`}>{'signature' in paragraph && paragraph.signature === 'sign' && index === previewParagraphs.length - 8 ? 'Signature: applied in downloaded PDF' : 'signature' in paragraph && paragraph.signature === 'date' && index === previewParagraphs.length - 7 ? 'Date signed: PDF generation date' : activeTab === 'sow' ? paragraph.text : renderText(paragraph.text)}</p>)}</div>
              ...
              <p className="preview-disclaimer">Text preview · Your signature is embedded in the downloaded PDF.</p>
